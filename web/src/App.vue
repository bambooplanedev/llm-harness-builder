...
const runTask = ref('')
/** The run this page started. Run and Abort are about it, not about any run opened from the list (a live proxy session among them). */
const ownRun = ref('')
const running = () => !!ownRun.value && runId.value === ownRun.value && !events.value.some(e => e.type === 'done')
...
function open(id: string, runTaskText: string) {
  unsub?.(); events.value = []; live.value = {}; runId.value = id; runTask.value = runTaskText
  unsub = api.events(id,
...
async function start() {
  error.value = ''
  try { const { runId: id } = await api.startRun(outbound(config.value), task.value, workdir.value); ownRun.value = id; open(id, task.value) }
  ...
  <div class="app">
    ...
    <div class="layout" v-else>
      ...
      <div class="col">
        ...
        <h4>Runs</h4>
        <div class="runs">
          <div v-for="r in runs" :key="r.id" @click="open(r.id, r.task)">
