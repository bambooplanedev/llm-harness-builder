...
after a CLI run). Diff two traces with any tool. The UI has two tabs: **Workbench** runs one
harness and shows its trace, **Bench** watches a `bench` run happening in another terminal — its
table, and the live trace of the run in flight.
